...
var configuration = new Configuration({
    apiKey: "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"
});
...
app.post("/message", function() {
    var _ref = _asyncToGenerator(function(req, res) {
        var json, body, embeddingResponse, _embeddingResponse_data_data, embedding, client, result, conversations, moderationResponse, moderation, e;
        return __generator(this, function(_state) {
            switch(_state.label){
                                ...
                                return __generator(this, function(_state) {
                                    switch(_state.label){
                                        case 0:
                                            ...
                                            return [
                                                4,
                                                client.query("select * from messages where multiple = false and session = $1 order by index", [
                                                    cv.session
                                                ])
                                            ];
                                        case 1:
                                            ...
                                            return [
                                                2,
                                                cr.rows.length > 0 ? "(User ".concat(cv.user, "'s messages to you at ").concat(new Date(cv.timestamp).toDateString(), ')\n"""\n') + cr.rows.map(function(row) {
                ...
                case 6:
                    ...
                    return [
                        4,
                        client.query('insert into messages (content, embedding, session, index, multiple, "user", timestamp) values ($1, $2, $3, $4, $5, $6, $7);', [
                            body,
                            "[" + embedding.toString() + "]",
                            json.session,
                            json.index,
